...
var glixl = (function(glixl)
{
    ...
    glixl.Scene.prototype.set_viewport = function(parameters)
    {
    ...
    {
        var col = Math.floor(tile.x / this.columns);
        var row = Math.floor(tile.y / this.rows);
    ...
    glixl.Scene.prototype.update = function()
    {
        ...
        for(var s=0 ; s<this.tiles.length ; s++)
        {
            // TODO: update tiles?
            x1 = this.tiles[s].x;
            y1 = this.tiles[s].y - this.tiles[s].z;
            x2 = this.tiles[s].x + this.tiles[s].width;
            y2 = this.tiles[s].y + this.tiles[s].height - this.tiles[s].z;
            z = this.tiles[s].z;
            ...
            tex_coords.push.apply( tex_coords, this.sprite_sheet.get_texture_coordinates(this.tiles[s].frame) );
        }
        
        this.context.bindBuffer(this.context.ARRAY_BUFFER, this.tile_vertex_buffer);
        this.context.bufferData(this.context.ARRAY_BUFFER, new Float32Array(vertices), this.context.DYNAMIC_DRAW);
        
        this.context.bindBuffer(this.context.ARRAY_BUFFER, this.tile_texture_buffer);
        this.context.bufferData(this.context.ARRAY_BUFFER, new Float32Array(tex_coords), this.context.DYNAMIC_DRAW);
